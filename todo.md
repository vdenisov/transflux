...
- [ ] Migrate `BranchDefImpl`, `DefaultBranchDefImpl`, and `ContextScopeImpl` to extend `ConfigurableDefImpl` directly. Each implements `defLabel()` to match the diagnostic phrasing the user expects: `BranchDefImpl` → `"branch '" + branchId + "'"`; `DefaultBranchDefImpl` → `"default branch"`; `ContextScopeImpl` → `"forContext scope for " + contextType.getSimpleName()`. The actual mutator-guarding happens in 2.6.12. (Deferred to 2.6.12 where these gain their guards.)
...
- [ ] Document the expected shape in the "DSL Shape" note in `CLAUDE.md` so Phase 3 `TriggerDefImpl` / `ListenerDefImpl` follow it. Note the two-base split explicitly: id-bearing defs extend `IdentifiedDefImpl`; id-less guarded defs extend `ConfigurableDefImpl` directly. (Deferred — bundled with 2.6.12 once the id-less defs actually extend the base and the full pattern is in the codebase.)

#### 2.6.11c — Polymorphic dispatch over sealed types (kill instanceof chains)
- [ ] `OperationDefImpl.buildBound(StateMachineImpl<T>)` — abstract method on the sealed base, with `SimpleOperationDefImpl` and `CompositeOperationDefImpl` overrides. Removes the 2-arm `instanceof` chain in `TransitionDefImpl.buildBoundOperation` (lines ~217-222 — `SimpleOperationDefImpl simple` / `CompositeOperationDefImpl composite`).
- [ ] `OperationDefImpl.checkRefs(scopeContext, scopeLabel, smDef)` — abstract method on the sealed base. `SimpleOperationDefImpl` no-ops; `CompositeOperationDefImpl` walks its `ActionRef` list (the current body of `StateMachineDefImpl.checkCompositeRefs`). Eliminates the `op instanceof CompositeOperationDefImpl` branch in `StateMachineDefImpl.validateContextCompatibilityAndCycles` (lines ~1112-1118).
- [ ] `OperationDefImpl.bindScope(stateMachine, rootRegistry, canonical, conditionRegistry)` and `OperationDefImpl.flattenScope()` — abstract methods on the sealed base. `SimpleOperationDefImpl` no-ops; `CompositeOperationDefImpl` does today's `bindCompositeScope` / scope-flatten body. Eliminates the two remaining `op instanceof CompositeOperationDefImpl` branches in `StateMachineDefImpl.bindCompositeScopes` (lines ~310-322) and `flattenCompositeScopes` (lines ~379-396). After this lands, `StateMachineDefImpl` walks transitions and SM-level composites uniformly and just calls `op.bindScope(...)` / `op.flattenScope()`.
- [ ] `ConditionDescriptor.resolve(registry, path)` — abstract method on the sealed `ConditionDescriptor`, each of the five record variants implements it. Flattens the 5-arm chain at `ConditionResolver.java:74-92`. `ConditionResolver.resolve` becomes a thin dispatch to `descriptor.resolve(...)`.
- [ ] `MapperRef.validateAgainst(scopeContext, scopeLabel, kind, memberId, componentContext, mapperRegistry)` — abstract method on `MapperRef`. Each variant (`PassThrough`, `ById`, `InlineFunction`, `InlineMapper`) knows how to validate itself. Flattens `StateMachineDefImpl.checkMemberRef` (lines ~1148-1184).
- [ ] `MapperRef.resolve(stateMachine)` returning `ResolvedContextMapping` — same pattern. Flattens the 4-arm chain in `CompositeOperationDefImpl.resolveMapping` (lines ~470-497).
...
- [ ] Add the matching `collectInlineRegistrations(sink)` to `BranchDefImpl` and `DefaultBranchDefImpl` (walking their own `actionRefs` list) and to `ConditionalStepDefImpl` (walking branches + default branch). Eliminates `ConditionalStepDefImpl.getInlineStepInstances()` / `getInlineStepClasses()` plus its private `collectInlineInstances` / `collectInlineClasses` helpers, in addition to the Composite-side accessors.
- [ ] Replace `CompositeOperationDefImpl.getInlineStepInstances()` / `getInlineStepClasses()` / `getInlineOperationInstances()` / `getInlineOperationClasses()` accessors and the `StateMachineDefImpl.bindCompositeScope` / `registerInlineSteps` / `registerInlineOperations` plumbing (lines ~398-507) with a single `composite.collectInlineRegistrations(sink)` walk. Eliminates the parallel-maps reconstruction on both the Composite and Conditional sides.
