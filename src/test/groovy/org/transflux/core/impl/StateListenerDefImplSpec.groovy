...
class StateListenerDefImplSpec extends Specification {
    ...
    static class CtorlessListener implements StateListener<Object> {
    ...
    def 'constructor rejects null id'() {
        when:
        new StateListenerDefImpl<Object>(null)
    ...
    def "constructor rejects blank id: '#id'"() {
        when:
        new StateListenerDefImpl<Object>(id)
    ...
    def 'id and metadata round-trip into the bound listener'() {
        given:
        def def_ = new StateListenerDefImpl<Object>('l1')
        def_.beginConfigurer()
        def_.withName('Audit').withDescription('records entries').using(new NoopListener())
    ...
    def 'using(instance) resolves to a BoundStateListener pointing at the instance'() {
        given:
    ...
    def 'buildBoundListener without using(...) names the listener and the fix'() {
        ...
        then:
        def e = thrown(TransfluxValidationException)
        e.message.contains("State listener 'l1'")
