...
- [ ] **Inline nested declarative containers in the Java DSL** - moved to [Phase 4b](phase-4b-action-sequence-grammar.md), which closes the whole parity gap rather than this one cell: a member-position `operation(id, ...)`, a conditional nested inside a branch, mappers and forks on branch members, and one shared member grammar (`ActionSequence`) the YAML mapper is written against. Nothing to do here beyond confirming it shipped before the mapper starts.
- [ ] **`ConditionDef<T, C>` (deferred from Phase 3.7).** Mandatory id, optional name/description, covering the four authoring flavours (instance, class, predicate, expression), plus the configurer overloads that depend on it — `StateMachineDef.condition(id, Consumer<ConditionDef>)` and `TransitionDef.preCondition(...)` / `postCondition(...)`. Phase 3 closed the item as a decision rather than as code: the def's only payload beyond what exists is metadata nothing reads (no `Condition` runtime view, no condition catalog, no payload carrying one), and making it reach runtime means widening all five `ConditionDescriptor` records plus `BoundCondition` and mirroring the family across `StateMachineDef`, `ContextScope`, `TransitionDef`'s two slots, `BranchDef`, `ManualTriggerDef` and `DataTriggerDef`. It lands here because the sealed descriptor grammar it intersects is what §5.5 pins down for YAML anyway. Naming is already settled by precedent: `condition(String, Consumer<…>)` collides with `condition(String, String)` and `condition(String, Predicate<T>)` for the same lambda-resolution reason that produced `conditionPredicate` / `conditionExpression` and `mapperDef`.
