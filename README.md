...
- `org.transflux.core.action` — `Action` (the single executable contract) and `ActionKind`, `ActionSequence` (the member grammar every ordered action list shares — a container, a conditional's branch, its default branch, and a transition's body), `Compensation`, `ContextMapper`, the def-side types (`ActionDef` and its `StepDef` / `OperationDef` forms, `ConditionalOperationDef`, `MapperDef`, `BranchDef`, `DefaultBranchDef`, `NoMatchBehavior`, `CompensationRouteDef`, `ForkableContext`, `ForkRejectionPolicy`), and the start/complete/error listener surface (`ActionListener`, its `ActionListenerDef` builder, and the `ActionExecution` / `ActionPhase` payload).
