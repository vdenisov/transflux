...
**Per-transition context.** Each `TransitionDef<T, C>` carries its own context type `C`. There are three ways to set it: (1) `StateDef.transitionsTo(target, id, Class<C>)` pre-binds the context inline during the state declaration; (2) calling `.usingContext(Class<C>)` on the obtained `TransitionDef` re-types it later; (3) leaving both unset defaults the transition to `Object.class` (accepts any non-null context, also accepts `null`). `Void.class` is a sentinel — a transition declared with `Void` context rejects any non-null firing value at the dispatch boundary (`EntityBindingImpl.verifyFireContext`). SM-level reusable components (steps, conditions, composite operations) are registered without a context constraint (`step(id, Step<T, ?>)`), or with an explicit context tag via the typed overloads (`step(id, Class<C>, Step<T, C>)`) or grouped under a `useContext(Class<C>, scope -> ...)` block; both authoring styles populate the same `componentContextTypes` map that the build-time context-compatibility check reads.
